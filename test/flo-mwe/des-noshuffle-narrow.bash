...
class NarrowDESIO extends Bundle {
    val key_lo        = Bits(INPUT, width = 32)
    val key_hi        = Bits(INPUT, width = InitialKeySize-32)
    val plaintext_lo  = Bits(INPUT, width = 32)
    val plaintext_hi  = Bits(INPUT, width = BlockSize-32)
    val ciphertext_lo = Bits(OUTPUT, width = 32)
    val ciphertext_hi = Bits(OUTPUT, width = BlockSize-32)
}

class NarrowDES extends Module {

    val io = new NarrowDESIO()

    val rounds = Vec.fill(NumRounds+1) { new DESRound() }
    val roundFunctions = ArrayBuffer.fill(NumRounds+1) { Module(new Feistel()) }

    val key = Cat(io.key_hi, io.key_lo)
    val plaintext = Cat(io.plaintext_hi, io.plaintext_lo)

    // initial round half-blocks swapped to keep loop operations consistent
    rounds(0).KeyRotationL := getBitsBE1(key, PC1L)
    rounds(0).KeyRotationR := getBitsBE1(key, PC1R)
    rounds(0).BlockR       := getBitsBE1(plaintext, IPL)
    rounds(0).BlockL       := getBitsBE1(plaintext, IPR)
    ...
    val ciphertext = getBitsBE1(Cat(rounds(NumRounds).BlockL, rounds(NumRounds).BlockR), FP)
    io.ciphertext_lo := ciphertext(31,0)
    io.ciphertext_hi := ciphertext(BlockSize-1, 32)
}

class NarrowDESTester(c: NarrowDES) extends Tester(c) {
  for (i <- 0 until 100) {
    poke(c.io.key_hi, BigInt(32, rnd))
    poke(c.io.key_lo, BigInt(32, rnd))
    poke(c.io.plaintext_hi, BigInt(32, rnd))
    poke(c.io.plaintext_lo, BigInt(32, rnd))
    step(1)
  }
}
...
object Utils {
    def getBitsBE1(in: Bits, positions: List[Int]) = Cat(in(15, 0), in)
