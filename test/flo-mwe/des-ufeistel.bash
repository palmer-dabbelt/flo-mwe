...
class DESSBoxTester(dut: DESSBox) extends Tester(dut) {
  for (i <- 0 until 10) {
    poke(dut.io.in, BigInt(6))
    step(1)
  }
}
...
class FeistelIO extends Bundle {
    ...
    val d_sbox_in = Vec.fill(8){ Bits(OUTPUT, width = 6) }
    val d_expanded = Bits(OUTPUT, width = 48)
    val d_xored = Bits(OUTPUT, width = 48)
}

class Feistel extends Module {
    val io = new FeistelIO()
    val expanded = Cat(io.halfBlock, io.halfBlock)(47, 0)
    io.d_expanded := expanded
    val xored = expanded ^ io.subkey
    io.d_xored := xored
    val iter = SBoxMaps.iterator
    val subs = ArrayBuffer.fill(8) { Module(new DESSBox(iter.next())) }
    for (i <- 0 until 8) { io.d_sbox_in(i) := xored(48 - (i*6), 42 - (i*6)) }
    for (i <- 0 until 8) { subs(i).io.in := xored(48 - (i*6), 42 - (i*6)) }
    val subbed = Cat(subs.map{ x => x.io.out })
    io.output := getBitsBE1(subbed, P)
}

class FeistelTester(dut: Feistel) extends Tester(dut) {
  for (i <- 0 until 100) {
    poke(dut.io.halfBlock, BigInt(BlockSize/2, rnd))
    poke(dut.io.subkey,    BigInt(SubkeySize,  rnd))
    step(1)
  }
}
